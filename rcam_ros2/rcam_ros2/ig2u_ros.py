...
from teleop_node import TeleopWrapper
...
SpeedCommandMax = 100
LinearVelocityMax = 1.0
AngularVelocityMax = 1.0

g_teleop = TeleopWrapper()
...
def handle_motion_command_ros(motComand, speedCommand):
    global g_teleop

    print("handle_motion_command_ros {0} {1}\r\n".format(motComand, speedCommand))

    assert (speedCommand >= 0) and (speedCommand <= SpeedCommandMax)

    linearVelocityMsecAbs =  LinearVelocityMax * speedCommand / SpeedCommandMax
    if motComand == "top":
        g_teleop.setLinearVelocity(linearVelocityMsecAbs)
    if motComand == "down":
        g_teleop.setLinearVelocity(-linearVelocityMsecAbs)
    if motComand == "stop":
        g_teleop.setLinearVelocity(0.)
        g_teleop.setAngularVelocity(0.)

    angularVelocity = 0.0
    if motComand == "left":
        g_teleop.setAngularVelocity(-AngularVelocityMax)
    if motComand == "right":
        g_teleop.setAngularVelocity(AngularVelocityMax)
    ...
    if mode == "mot":
     try:
      if inRasbery:
       ...
       handle_motion_command_ros(g_mot_command, g_speed_command)
...
def handle_keypress(key):
    ...
    SpeedDic = {
        ord('+') : lambda cmdVal : min(cmdVal  + 10, SpeedCommandMax),
